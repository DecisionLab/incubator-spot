...
CreateStoryboardInputType = GraphQLInputObjectType(
    name='ADCreateStoryboardInputType',
    fields={
        'date': GraphQLInputObjectField(
            type=SpotDateType,
        ...
        'threatDetails': GraphQLInputObjectField(
            ...
            description='Threat details. See ADThreatInformation.details'
        ),
...
def _score_connections(args):
    ...
    for cmd in _input:
        ...
        result = AD.score_request(date=_date, score=score, uri=uri)
...
def _create_storyboard(args):
    ...
    result = AD.create_storyboard(date=_date, uri=uri, title=title, text=text, expanded_search=threat_details, top_results=first)
...
def _reset_scored_connections(args):
    ...
    result = AD.reset_scored_connections(date=_date)
